class FixedAssets::CommoditiesController < AdministrationController
  ...
  # GET /commodities
  def index
    @commodities = Company.current.commodities
  ...
  # GET /commodities/new
  def new
    @commodity = Company.current.commodities.build
  end
  ...
  # POST /commodities
  def create
    @commodity = Company.current.commodities.build(commodity_create_params)
  ...
  private
    ...
    def find_resource
      @commodity = Company.current.commodities.find(params[:commodity_id] || params[:id])
    end

    def find_voucher_row
      @voucher_row = Company.current.voucher_rows.find(params[:voucher_row_id])
    end
    ...
    def commodity_sales_params
      ...
      @commodity.profit_account = Company.current.accounts.find_by(tilino: params[:profit_account])
      @commodity.sales_account = Company.current.accounts.find_by(tilino: params[:sales_account])
