class Woo::Orders < Woo::Base
  ...
  def fetch(orders_path:, customer_id: nil, order_status: 'processing', order_status_after:)
    ...
    response.each do |order|
      ...
      #check status on the order and if pending, skip if not an invoice order
      logger.info "Status: #{order['status']} Payment method: #{order['payment_method']}"
      if order['status'] == "pending" && order['payment_method'] != "lasku_gateway"
        logger.info "#{order['id']} on maksamista odottava luottkorttimaksu, ohitetaan!"
        next
      end
  ...
  def write_to_file(order)
    ...
    if customer_id == "b2b"
      if order['payment_method'] == "lasku_gateway"       
         filepath = File.join(edi_orders_path, "invoice-order-#{order['id']}.txt")
      else
